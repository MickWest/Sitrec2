...
for (const scenario of ["straight", "raised ridge", "lowered ridge", "overlapping ridges", "mirage fold"]) {
    test(`refraction preserves the analytic ${scenario}`, async ({page}) => {
        const result = await page.evaluate(({fragment, projection, inverseProjection, scenario}) => {
            const height = 512, width = 1, ridge = 0.625, distance = 1000, maxDistance = 5000;
            const canvas = document.createElement("canvas");
            canvas.width = width; canvas.height = height;
            const gl = canvas.getContext("webgl2");
            if (!gl || !gl.getExtension("OES_texture_float_linear")) throw new Error("Float texture filtering required");
            ...
            const curvature = ["raised ridge", "overlapping ridges"].includes(scenario) ? -4e-6 : scenario === "lowered ridge" ? 4e-6 : 0;
            ...
            texture("tRays", 3, columns, rows, rays, true, true);
