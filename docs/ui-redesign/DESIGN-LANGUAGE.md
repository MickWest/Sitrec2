...
- **Hover-reveal.** The header + per-view controls appear on pointer-hover over the view and hide on leave.
- **Pin.** A pin button on the header keeps it (and the per-view UI) shown regardless of hover.
