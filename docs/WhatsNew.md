...
### New Features
- **Physically based aerial perspective**: the look view now composites distant geometry as `scene * T + sky_ray_color * (1 - T)`, where `T` is per-ray transmittance through an exponential atmosphere and `sky_ray_color` is the same per-ray sky gradient that paints the background. Distant buildings and terrain converge toward the color of the sky directly behind them instead of a fixed grey fog tint, so a city at the horizon under a yellow sunset goes yellow, under a blue overhead it goes blue. The *Atmo Visibility* slider is now mapped to physical extinction via Koschmieder's formula (`beta = 3.912 / visibility_meters`), and a normalized ray-distance prepass replaces hardware perspective depth so Earth-scale far planes do not collapse the precision available for haze integration. Surface-level views, high-altitude aircraft views, and near-space / satellite views all use the same model — a Rayleigh/aerosol scale-height integration with a tangent-ray limb-haze boost so long horizontal sightlines from space accumulate the dense lower atmosphere without an extra pass, while vertical-up sightlines correctly thin out. A continuous angle weight replaces the prior hard slant threshold near 364,354 ft so angled-down high-altitude views no longer pop. An analytic Earth-surface/horizon air-mass fallback covers shallow sightlines into 3D Tiles where the lightweight depth prepass under-reports custom-positioned tile geometry. The feature is gated to lookView + Atmosphere + (Horizon Haze or Sky Gradient) and stays a no-op for IR/XR/legacy day-sky paths; the shader and distance render target are released when disabled. See `docs/AtmosphericAerialPerspective.md` for the full physical basis, controls, and limits.
- **Atmosphere horizon haze gradient**: independent per-ray horizon haze gradient rendered in `CNodeView3D` and driven by the existing `CNodeSunlight` solar geometry — distinct from the legacy uniform-fog horizon and used as the sky-side input to the new aerial perspective composite above. Adds the underlying machinery that makes the new aerial perspective look physically grounded rather than a tinted fog band.
- **Terrain self-shadowing**: terrain tiles now cast and receive their own shadows, so ridgelines, hills, and crater walls darken the ground in their lee when sun shadows are enabled. Wired through `CNodeTerrain` and the `QuadTreeTile` shadow flags so each LOD tile participates without re-allocating geometry.

### Improvements
- **Reorganized Lighting menu defaults**: the Lighting menu and its sun/sky/ambient defaults were re-laddered for clarity — better grouping in `CNodeLighting`, label and tooltip cleanup across locales (`i18n/en.js`), and per-view wiring updates in `CNodeView3D` so the new aerial-perspective + horizon-gradient toggles read naturally alongside the existing shadow controls.
- **Stabilized video exports embed video info**: the stabilized-video export path now includes the same on-screen video-info overlay (frame number, time, source metadata) that the regular `CNodeVideoInfoUI` renders, so a stabilized clip dropped into another tool keeps its provenance. The video-info UI itself was simplified — ~130 lines of redundant rendering scaffolding collapsed into the shared compositor in `CObjectTracking`.

### Bug Fixes
- **Motion panorama export no longer duplicates frames**: the export walked the frame range with a play-loop-style head that could re-emit the same decoded frame when the decode pipeline stalled, producing zip files where consecutive output frames were byte-identical. The new export driver in `CMotionAnalysis` actively walks `[aFrame..bFrame]`, retries incomplete frames after a buffer warm-up, and treats each output index as a one-shot deliverable so the panorama assembly sees the correct optical-flow displacement between frames.
