...
the output directory (`benchmarks/botbench/results/`) is deliberately not in
version control: anyone with the repository regenerates an identical set.
...
For the Mundane, Extreme, and Anomalies v1 datasets, see
[Build the Mundane, Extreme, and Anomalies datasets](BuildingMotionDatasets.md) for a
complete clean-machine script, full or single selections, 1 Hz and 10 Hz
options, and parameter reference. `mundane_v1` retains rock_v3's 100 balloons,
100 drones, and 100 weather balloons, using the new platform paths and centered
crops of shared 300-second masters.
...
The sets follow five design decisions:
...
- **The maneuver taxonomy is partitioned by anomaly, not by shape.** One
  ...
  folder. The split lets the two questions be scored separately.
...
- **The balloon sets split on the sensor's path, with duration pinned at 20 s.**
  The maneuver sets vary clip length; the balloon sets vary the sensor path —
  fly-past, gentle bank, full orbit — with length fixed at 20 s. The orbit gives
  the largest [parallax](BOTBench.md#parallax) of the three paths. The twenty variants are five
...
11° and 114° as seen from the object, and no cell falls below 10°. Each
...
camera switch and the target track is drawn.
