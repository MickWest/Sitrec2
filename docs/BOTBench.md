...
twenty scenarios at [1 Hz](#hertz-hz), spanning the space the analysis is
supposed to handle: balloons on orbiting, straight and curving sensor paths at
2–20 km; a high-altitude balloon at 50 km; a slow [aerostat](#aerostat);
...
[20 g](#g-force-g) pulse, each paired with a mundane twin on the same sensor
...
Two commands write five [**botsets**](#botset), 456 scenarios in all, at 10 Hz:
...
- **The two families use different error models, on purpose.** The maneuver
  ladder is operator [wobble](#correlated-wobble): a seeded random walk that
  recentres, so it is zero-mean — the kind of error a fit absorbs best. The
  balloon ladder is one-way [**drift**](#drift-error): a slow slide off the
...
- **The balloon sets split on the sensor's path, with duration pinned at 20 s.**
  ...
  bound on what [parallax](#parallax) can buy. The twenty variants are five
