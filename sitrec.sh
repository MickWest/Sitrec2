...
" 2>/dev/null)
        ...
        ;;
    bake)
        # Bake the env vars from a .env file into a NEW image, derived from the
        # published Sitrec image, so the result is self-configured and can be
        # stored in a registry. No rebuild is needed: the published image's
        # entrypoint reads environment variables at container start, so all we
        # do is add `ENV` layers on top of it. (Runtime -e/env_file still
        # overrides these baked-in values, since ENV is the lowest priority.)
        shift
        ENV_FILE=".env"
        BASE_TAG="latest"
        PUSH=false
        ...
        if [ ! -f "$ENV_FILE" ]; then
            echo "[sitrec] ERROR: env file '$ENV_FILE' not found."
            exit 1
        fi

        # Pick the container runtime (docker or podman) from the compose command.
        case "$COMPOSE" in
            docker*) RUNTIME_CMD="docker" ;;
            *)       RUNTIME_CMD="podman" ;;
        esac

        BASE_IMAGE="${IMAGE}:${BASE_TAG}"

        echo "[sitrec] Baking '$ENV_FILE' into $BASE_IMAGE  ->  $TARGET"
        echo "[sitrec] WARNING: every value in '$ENV_FILE' is embedded in the image as"
        echo "          build-time ENV layers. Anyone who can pull '$TARGET' (or read its"
        ...
        {
            echo "# Auto-generated by ./sitrec.sh bake — do not edit."
            echo "# Bakes '$ENV_FILE' into $BASE_IMAGE so the image is self-configured."
            echo "FROM ${BASE_IMAGE}"
        } > "$DF"

        # Parse the env file into ENV lines, mirroring the entrypoint's behaviour:
        # skip blanks/comments/empty values, allow an optional 'export ', split
        # on the first '=', and strip one layer of surrounding quotes. Values are
        # then re-quoted for Dockerfile syntax with backslashes, double-quotes,
        # and '$' escaped (so Docker doesn't try to expand them).
        #
        ...
        while IFS= read -r line || [ -n "$line" ]; do
            ...
            [ -z "$val" ] && continue                         # skip empty values (entrypoint does too)
            esc="${val//\\/\\\\}"                             # escape backslashes
        ...
        if [ "$BAKED_COUNT" -eq 0 ]; then
            echo "[sitrec] ERROR: no usable KEY=value lines found in '$ENV_FILE'."
            exit 1
        fi
        ...
        if [ "$PUSH" = true ]; then
            ...
            echo "[sitrec] Pushed $TARGET"
        else
            echo "[sitrec] Not pushed (no --push). To push it yourself:"
            echo "           $RUNTIME_CMD push $TARGET"
        fi
        ;;
    update)
