// Add synchronous KLV to a single-program H.264 transport stream.
...
export function muxVideoKlv(input, records, fps = 30, {klvPID = 0x101, metadataSamples,
    allowAudio = false, sequenceCounter = true, tablePacketPeriod = 0, additionalStreams = []} = {}) {
    ...
    const streams = [{pid: klvPID, records}, ...additionalStreams]
        .map((stream, service) => ({...stream, service, cc: 0, emitted: 0}));
    if (streams.some(s => s.records.length !== records.length || !Number.isInteger(s.pid)
        || s.pid < 32 || s.pid >= 0x1fff) || new Set(streams.map(s => s.pid)).size !== streams.length) {
        throw new Error("Invalid metadata streams");
    }
    let pmtPID, videoPID, firstPTS, pmtCount = 0, emitted = 0;
    function emit(klvBytes, pts, stream = streams[0]) {
        const klv = Buffer.from(klvBytes);
        if (klv.length > 65522) throw new Error("KLV record exceeds one PES packet");
        // Complete, random-access metadata AU; each stream has its own service.
        const au = Buffer.from([stream.service, sequenceCounter ? stream.emitted & 255 : 0, 0xdf, klv.length >> 8, klv.length & 255]);
        ...
        for (let at = 0; at < pes.length;) {
            ...
            out[0] = 0x47; out[1] = (at === 0 ? 0x40 : 0) | (stream.pid >> 8); out[2] = stream.pid & 255;
            out[3] = (take < 184 ? 0x30 : 0x10) | (stream.cc++ & 15);
            const dest = 188 - take;
            if (take < 184) { out[4] = 183 - take; if (out[4]) out[5] = 0; }
            pes.copy(out, dest, at, at + take); output.push(out); at += take;
        }
        stream.emitted++;
        if (stream === streams[0]) emitted++;
    }
    for (let offset = 0; offset < ts.length; offset += 188) {
        ...
        if (streams.some(s => s.pid === pid)) throw new Error("Metadata PID already in use");
        if (pid === 0 && start) {
        ...
        if (pid === pmtPID) {
            ...
            // with a separate service per stream. Both descriptors aid readers.
            const entries = streams.flatMap(stream => {
                const descriptors = Buffer.from([5,4,75,76,86,65, 0x26,9,1,0,0xff,75,76,86,65,stream.service,0x0f]);
                return [Buffer.from([0x15,0xe0 | (stream.pid >> 8),stream.pid & 255,0xf0,descriptors.length]), descriptors];
            });
            const replacement = Buffer.concat([section, ...entries, Buffer.alloc(4)]);
        ...
        for (const stream of streams.slice(1)) {
            if (stream.records[f]?.klv) emit(stream.records[f].klv, pts, stream);
        }
    }
