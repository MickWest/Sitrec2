// Read TLE records field by field, by their GRAMMAR, never by column.
//
// A TLE is a fixed-column format, and TLE text rarely keeps its columns once it
// has been through a browser, a chat window, a PDF, or another program's writer:
//
...
//   - some programs write an element set with single spaces between fields and
//     the optional international designator left out, so line 1 has one field
//     fewer than the fixed layout expects.
//
...
//
// Two callers share it: TLEPaste.js, for text pasted into the app, and CTLEData
// (TLEUtils.ts), for a TLE file whose lines cannot be laid out on the fixed
// columns. It is pure — no Globals, no FileManager, no dialogs — because
// TLEUtils.ts is loaded directly by unit tests.
 ...
 * name — readTLERecords treats anything that fails to parse as a record as name
...
/**
 * Read every TLE record in a piece of text.
 *
 * @param {string} text
 * @returns {{records: {name: string, line1: string, line2: string}[], errors: string[]}}
 *   The records, as canonical 69-character lines; and, for anything that had the
 *   shape of a record but did not read as one, the reason, in the order found.
 *   No records and no errors means the text holds no element lines at all.
 */
export function readTLERecords(text) {
...
export function parsePastedTLE(text) {
    ...
    const {records, errors} = readTLERecords(text);
