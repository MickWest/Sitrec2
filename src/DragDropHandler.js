...
import {parseMapURL} from "./CoordinateParser";
    ...
    async uploadURL(url, options = {}) {
        ...
        try {
            ...
            if (!isSubdomain(urlObject.hostname, SITREC_DOMAIN)
                ...
                // (Google Maps, ADS-B Exchange, Flightradar24 - see parseMapURL)
                const mapLocation = parseMapURL(url);
                if (mapLocation) {
                    // Where the URL says how much ground the view covered, put
                    // the camera high enough to show the same span; otherwise a
                    // default altitude (metres).
                    let alt = 30000;
                    if (mapLocation.verticalSpanM !== undefined) {
                        const vFOV = radians(NodeMan.get("mainCamera").camera.fov);
                        alt = mapLocation.verticalSpanM / 2 / Math.tan(vFOV / 2);
                    }
                    console.log("Map URL detected, extracting location: " + mapLocation.lat + ", " + mapLocation.lon + " Altitude: " + alt);
                    closeURLProgress();
                    this.droppedLLA(mapLocation.lat, mapLocation.lon, alt)
                    return;
                }
