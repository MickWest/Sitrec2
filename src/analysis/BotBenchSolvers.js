...
export const SOLVERS = Object.freeze([
    ...
    {id: "fixedPoint", name: "Stationary Point / Fixed Point in the Sky", shortName: "fixed_point", group: "Geometry checks", units: [],
        note: "A stationary object, or a fixed direction at infinity; closed-form."},
    {id: "gfCV", name: "Global Fit: Constant Velocity", shortName: "cv", group: "Curve fits", units: ["gfCV"],
        note: "Direct least-squares constant-velocity fit to all sightlines. Cheap."},
    {id: "gfCA", name: "Global Fit: Constant Acceleration", shortName: "ca", group: "Curve fits", units: ["gfCA"],
        note: "Direct least-squares constant-acceleration fit to all sightlines. Cheap."},
    {id: "gfKalman", name: "Global Fit: Kalman Smoother", shortName: "kf", group: "Curve fits", units: ["kalman"],
        note: "The Kalman-smoothed sightline fit, as the live analysis offers it. Cheap."},
    ...POLY_ORDERS.map((order) => ({
        id: `gfPolyALS:${order}`, name: `Global Fit: Polynomial LSQ (order ${order})`, shortName: `poly_${order}`, group: "Curve fits",
        units: ["polySweep"], note: order === 1 ? "The deterministic alternating least-squares curve fits, one sweep for all five orders." : null,
    })),
    ...MONTE_CARLO_IDS.map(id => ({id, name: monteCarloName(id), shortName: id, group: "Monte Carlo (GPU)",
